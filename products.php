<?php
    ...
    include "./instalacion/conexion.php";
    $consultaGeneral = $conect->prepare('SELECT * FROM producto');
    $consultaGeneral->execute();
    $productos = $consultaGeneral->fetchAll();
?>
<!DOCTYPE html>
<html :class="{ 'theme-dark': dark }" x-data="data()" lang="en">
  <head>
    ...
    <title>Products - Nova Systems</title>
            ...
            <div class="w-full overflow-hidden rounded-lg shadow-xs">
              <div class="w-full overflow-x-auto">
                <table id="table" class="w-full whitespace-no-wrap">
                  ...
                  >
                  <?php foreach ($productos as $datos => $valor):?>
                    <tr class="text-gray-700 dark:text-gray-400">
                      <td class="px-4 py-3">
                        <div class="flex items-center text-sm">
                            <p class="font-semibold"><?= $valor['codigo']; ?></p>
                        </div>
                      </td>
                      <td class="px-4 py-3 text-sm">
                      <?= substr($valor['descripcion'],0,70); ?>
                      </td>
                      <td class="px-4 py-3 text-sm">
                      <?= $valor['precio']; ?>
                      </td>
                      ...
                      <td class="px-4 py-3">
                        <div class="flex items-center space-x-4 text-sm">
                          <a
                            ...
                            aria-label="Edit"
                            href="./edit_product.php?id_producto=<?= $valor['id_producto'] ?>"
                          >
                            <svg
                              class="w-5 h-5"
                              aria-hidden="true"
                          ...
                          <a
                            class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                            aria-label="Delete"
                            href="./inactivo.php?id_producto=<?= $valor['id_producto'] ?>"
                          >
                            <svg
                              class="w-5 h-5"
                              aria-hidden="true"
